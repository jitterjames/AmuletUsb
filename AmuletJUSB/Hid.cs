...
{    
    ...
    {         
			...
			try
			{
				...
				if ((result != 0))
				{
					...
					*/
					...
					//  Bugfix: was previously assuming vcSize was in bytes, but it's actually a count
					//  of the number of array elements (each element being one HidP_Value_Caps structure)
					//  that is returned. Tricky to calculate the size of such a structure exactly, so
					//  we play it safe and assume it's 1K; better to err on the side of large.
					//  (Actual value is around 50-200 bytes, so plenty of leeway).
					//  With the old bug, the array would be allocated too short, causing memory corruption.
					//
					//  In fact, since we don't use the result anyway, we can just comment the
					//  whole lot out.
					//                             -- EC, 12 Nov 2010
					//

					// Int32 vcSize = Capabilities.NumberInputValueCaps;				
					// Byte[] valueCaps = new Byte[vcSize * 1024];
        ...
        ///  <summary>
        ///  Retrieves a string corresponding to the manufacturer's name
        ///  </summary>
        ///  
        ///  <param name="hidHandle"> a handle to a device. </param>
        ///  
        ///  <returns>
        ///  An HIDP_CAPS str        
        internal string GetManufacturerDetails(SafeFileHandle hidHandle ) 
        {             
			Int32 bufSize = 200;
			...
			try
			{
				...
				if (HidD_GetManufacturerString(hidHandle, buffer, bufSize))
				{
